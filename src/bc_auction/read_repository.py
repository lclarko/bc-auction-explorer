...
class AuctionReadRepository:
    ...
    def list_listings(self, filters: ListingFilters) -> ListingPage:
        statement = self._current_listing_statement().where(*self._listing_conditions(filters))
        with self._engine.connect() as connection:
            total = int(
                connection.scalar(select(func.count()).select_from(statement.subquery())) or 0
            )
            rows = connection.execute(
                statement.order_by(*self._listing_order(filters.sort))
    ...
    def get_listing(self, source_id: str) -> ListingDetail | None:
        statement = self._current_listing_statement().where(auction_items.c.source_id == source_id)
        with self._engine.connect() as connection:
            row = connection.execute(statement).mappings().one_or_none()
        return _listing_detail_from_row(row) if row is not None else None

    def list_locations(self, *, limit: int) -> FacetList:
        location = func.coalesce(
            auction_items.c.location_canonical,
            auction_items.c.location_raw,
        ).label("value")
        statement = (
            select(location, func.count(auction_items.c.id).label("count"))
    ...
    def list_categories(self, *, limit: int) -> FacetList:
        category = func.coalesce(
            auction_items.c.category_canonical,
            auction_items.c.category_raw,
        ).label("value")
        statement = (
            select(category, func.count(auction_items.c.id).label("count"))
    ...
    @staticmethod
    def _current_listing_statement() -> Select[tuple[Any, ...]]:
        candidates = (
            select(
    ...
    @staticmethod
    def _listing_conditions(filters: ListingFilters) -> list[ColumnElement[bool]]:
        ...
        if filters.status is not None:
            conditions.append(auction_items.c.status == filters.status.value)
